...
if (!kidDashContent.includes('ACHIEVEMENTS')) {
  kidDashContent = kidDashContent.replace(/import \{ checkAchievement \} from "\.\.\/achievements";/, 'import { checkAchievement, ACHIEVEMENTS } from "../achievements";');
}
...
kidDashContent = kidDashContent.replace(/\{activeTab === "achievements" && \([\s\S]*?Мои Достижения[\s\S]*?\}\)[\s\S]*?(?=\{activeTab === "games")/, ''); // cleanup if exists
kidDashContent = kidDashContent.replace(/\{activeTab === "achievements" && \([\s\S]*?Раздел находится в разработке\.\.\.[\s\S]*?\}\)/, achievementsTab);
