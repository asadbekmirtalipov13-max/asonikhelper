...
const faqStateStr = `const [openFaq, setOpenFaq] = useState<string | null>(null);`;
                ...
                {(settings.faqs || []).map((faq) => (
                  <div key={faq.id} className="border border-slate-200 rounded-2xl overflow-hidden bg-slate-50">
                    <button
                      ...
                      className="w-full text-left p-4 font-bold text-slate-700 text-sm flex justify-between items-center hover:bg-slate-100 transition-colors"
                    >
                      {faq.question}
...
// Add missing lucide import if not there
