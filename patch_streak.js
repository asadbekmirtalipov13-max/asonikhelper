...
// Replace todayStr logic with GMT+5
...
content = content.replace(
  /if \(!isFree\) \{\n\s+updatedFields\.points = currentUser\.points - cost;\n\s+\}/,
  `if (!isFree) {
        updatedFields.points = currentUser.points - cost;
      }`
);

// In handleDailyCheckIn, add 29th day chest logic:
content = content.replace(
  /if \(newStreak === 15 \|\| newStreak === 30\) \{[\s\S]+?isChestDay = true;\n\s+\}/,
